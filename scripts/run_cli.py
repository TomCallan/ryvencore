...
def main():
    ...
    parser.add_argument('project_file', nargs='?', default='../saved_flows/flow_project.json', help='Path to flow project file')
    ...
    if not os.path.exists(filepath):
        ...
        saved_path = os.path.join(PROJECT_ROOT, 'saved_flows', filepath)
        if os.path.exists(saved_path):
            filepath = saved_path
        elif not filepath.endswith('.json'):
            saved_path_json = os.path.join(PROJECT_ROOT, 'saved_flows', f"{filepath}.json")
    ...
    if args.benchmark:
        ...
        metrics_path = os.path.join(PROJECT_ROOT, 'cli_benchmark_results.json')
