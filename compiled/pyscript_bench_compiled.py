# Structure Hash: 385152ace2c907f96a2888472638e9b8afe9524f626285eff4241c87f1dc7b8b
"""
Compiled ryvencore flow: pyscript_bench
...
def setup_flow(actual_nodes=None):
    ...
    # Node: Trigger (ID: 78)
    n_78 = TriggerNode(params=None)
    n_78.node_id = 78
    n_78.global_id = 78
    n_78.flow_alg = flow_alg
    if actual_nodes and 78 in actual_nodes:
        n_78.actual_node = actual_nodes[78]
    n_78.create_output('out', 'exec')
    n_78.loop_enabled = False
    n_78.loop_interval = 1.0
    n_78.wait_until_complete = False
    n_78.auto_exec_downstream = False
    nodes[78] = n_78

    # Node: Execution Timer (ID: 79)
    n_79 = ExecutionTimerNode(params=None)
    n_79.node_id = 79
    n_79.global_id = 79
    n_79.flow_alg = flow_alg
    if actual_nodes and 79 in actual_nodes:
        n_79.actual_node = actual_nodes[79]
    n_79.create_input('trigger', 'exec', default=rc.Data(None))
    n_79.create_output('out', 'exec')
    n_79.create_output('time_ms', 'data')
    n_79.loop_enabled = False
    n_79.loop_interval = 1.0
    n_79.wait_until_complete = False
    n_79.auto_exec_downstream = False
    nodes[79] = n_79

    # Node: Python REPL (ID: 80)
    n_80 = PythonReplNode(params=None)
    n_80.node_id = 80
    n_80.global_id = 80
    n_80.flow_alg = flow_alg
    if actual_nodes and 80 in actual_nodes:
        n_80.actual_node = actual_nodes[80]
    n_80.create_input('in1', 'data', default=rc.Data(0.0))
    n_80.create_input('in2', 'data', default=rc.Data(0.0))
    n_80.create_output('sum', 'data')
    n_80.create_output('prod', 'data')
    n_80.create_output('diff', 'data')
    n_80.create_output('ratio', 'data')
    n_80.loop_enabled = False
    n_80.loop_interval = 1.0
    n_80.wait_until_complete = False
    n_80.code = '\nclass Inputs:\n    in1 = 0.0\n    in2 = 0.0\nclass Outputs:\n    sum = 0.0\n    prod = 0.0\n    diff = 0.0\n    ratio = 0.0\nsum = in1 + in2\nprod = in1 * in2\ndiff = in1 - in2\nratio = in1 / (in2 + 0.001)\n    '
    n_80.auto_exec_downstream = False
    nodes[80] = n_80

    # Node: Log (ID: 76)
    n_76 = LogNode(params=None)
    n_76.node_id = 76
    n_76.global_id = 76
    n_76.flow_alg = flow_alg
    if actual_nodes and 76 in actual_nodes:
        n_76.actual_node = actual_nodes[76]
    n_76.create_input('msg', 'data', default=rc.Data(''))
    n_76.create_output('out', 'data')
    n_76.loop_enabled = False
    n_76.loop_interval = 1.0
    n_76.wait_until_complete = False
    n_76.auto_exec_downstream = False
    nodes[76] = n_76

    # Node: Log (ID: 77)
    n_77 = LogNode(params=None)
    n_77.node_id = 77
    n_77.global_id = 77
    n_77.flow_alg = flow_alg
    if actual_nodes and 77 in actual_nodes:
        n_77.actual_node = actual_nodes[77]
    n_77.create_input('msg', 'data', default=rc.Data(''))
    n_77.create_output('out', 'data')
    n_77.loop_enabled = False
    n_77.loop_interval = 1.0
    n_77.wait_until_complete = False
    n_77.auto_exec_downstream = False
    nodes[77] = n_77

    # Connections
    nodes[78].connections[0] = [(nodes[79], 0)]
    nodes[79].connections[1] = [(nodes[77], 0)]
    nodes[80].connections[0] = [(nodes[76], 0)]

    # Run initial placement events
    nodes[78].after_placement()
    nodes[79].after_placement()
    nodes[80].after_placement()
    nodes[76].after_placement()
    nodes[77].after_placement()
