# Structure Hash: 6526adf83b033a219744f4f2a1dc4e347d78460014a43eb095f98707a62303bf
"""
Compiled ryvencore flow: parquet_db_bench
...
def setup_flow(actual_nodes=None):
    ...
    # Node: Trigger (ID: 49)
    n_49 = TriggerNode(params=None)
    n_49.node_id = 49
    n_49.global_id = 49
    n_49.flow_alg = flow_alg
    if actual_nodes and 49 in actual_nodes:
        n_49.actual_node = actual_nodes[49]
    n_49.create_output('out', 'exec')
    n_49.loop_enabled = False
    n_49.loop_interval = 1.0
    n_49.wait_until_complete = False
    n_49.auto_exec_downstream = False
    nodes[49] = n_49

    # Node: Execution Timer (ID: 50)
    n_50 = ExecutionTimerNode(params=None)
    n_50.node_id = 50
    n_50.global_id = 50
    n_50.flow_alg = flow_alg
    if actual_nodes and 50 in actual_nodes:
        n_50.actual_node = actual_nodes[50]
    n_50.create_input('trigger', 'exec', default=rc.Data(None))
    n_50.create_output('out', 'exec')
    n_50.create_output('time_ms', 'data')
    n_50.loop_enabled = False
    n_50.loop_interval = 1.0
    n_50.wait_until_complete = False
    n_50.auto_exec_downstream = False
    nodes[50] = n_50

    # Node: DuckDB Query (ID: 51)
    n_51 = DuckDBQueryNode(params=None)
    n_51.node_id = 51
    n_51.global_id = 51
    n_51.flow_alg = flow_alg
    if actual_nodes and 51 in actual_nodes:
        n_51.actual_node = actual_nodes[51]
    n_51.create_input('database', 'data', default=rc.Data(':memory:'))
    n_51.create_input('query', 'data', default=rc.Data('SELECT 1 as val'))
    n_51.create_output('info', 'data')
    n_51.create_output('data', 'data')
    n_51.loop_enabled = False
    n_51.loop_interval = 1.0
    n_51.wait_until_complete = False
    n_51.auto_exec_downstream = False
    nodes[51] = n_51

    # Node: Log (ID: 47)
    n_47 = LogNode(params=None)
    n_47.node_id = 47
    n_47.global_id = 47
    n_47.flow_alg = flow_alg
    if actual_nodes and 47 in actual_nodes:
        n_47.actual_node = actual_nodes[47]
    n_47.create_input('msg', 'data', default=rc.Data(''))
    n_47.create_output('out', 'data')
    n_47.loop_enabled = False
    n_47.loop_interval = 1.0
    n_47.wait_until_complete = False
    n_47.auto_exec_downstream = False
    nodes[47] = n_47

    # Node: Log (ID: 48)
    n_48 = LogNode(params=None)
    n_48.node_id = 48
    n_48.global_id = 48
    n_48.flow_alg = flow_alg
    if actual_nodes and 48 in actual_nodes:
        n_48.actual_node = actual_nodes[48]
    n_48.create_input('msg', 'data', default=rc.Data(''))
    n_48.create_output('out', 'data')
    n_48.loop_enabled = False
    n_48.loop_interval = 1.0
    n_48.wait_until_complete = False
    n_48.auto_exec_downstream = False
    nodes[48] = n_48

    # Connections
    nodes[49].connections[0] = [(nodes[50], 0)]
    nodes[50].connections[1] = [(nodes[48], 0)]
    nodes[51].connections[0] = [(nodes[47], 0)]

    # Run initial placement events
    nodes[49].after_placement()
    nodes[50].after_placement()
    nodes[51].after_placement()
    nodes[47].after_placement()
    nodes[48].after_placement()
